...
with DAG(
    ...
    validate_input = KubernetesPodOperator(
        ...
        arguments=[
            "import os, sys; "
            # AIRFLOW_CTX_LOGICAL_DATE is set by Airflow: YYYY-MM-DDTHH:MM:SS+00:00
            # Fall back to today if not running inside Airflow.
            "raw = os.environ.get('AIRFLOW_CTX_LOGICAL_DATE', ''); "
            "d = raw[:10] if raw else __import__('datetime').date.today().isoformat(); "
